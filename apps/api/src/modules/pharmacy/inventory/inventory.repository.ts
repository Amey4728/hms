...
const medicineWithBatches = Prisma.validator<Prisma.MedicineInclude>()({
  batches: { where: { deletedAt: null }, select: { id: true, quantity: true, expiryDate: true, batchNumber: true } },
});
export type MedicineWithBatches = Prisma.MedicineGetPayload<{ include: typeof medicineWithBatches }>;

const batchWithMedicine = Prisma.validator<Prisma.MedicineBatchInclude>()({
  medicine: { select: { code: true, name: true } },
});
export type BatchWithMedicine = Prisma.MedicineBatchGetPayload<{ include: typeof batchWithMedicine }>;
...
export class InventoryRepository {
  ...
  async findMedicinesWithBatches(params: {
    ...
    const [items, total] = await this.prisma.$transaction([
      this.prisma.medicine.findMany({ where, include: medicineWithBatches, orderBy: { name: 'asc' }, skip: params.skip, take: params.take }),
  ...
  findAllMedicinesWithBatches(): Promise<MedicineWithBatches[]> {
    return this.prisma.medicine.findMany({ where: { deletedAt: null, isActive: true }, include: medicineWithBatches });
  }
