...
export class HrService {
  ...
  private async assertEmployee(employeeId: string) {
    if (!(await this.repo.findEmployee(employeeId))) throw new NotFoundException('Employee not found');
  }
  ...
  async listAttendance(employeeId: string, from?: string, to?: string) {
    await this.assertEmployee(employeeId);
    return this.repo.listAttendance(employeeId, from ? dateOnly(from) : undefined, to ? dateOnly(to) : undefined);
  }
  ...
  async listLeave(query: { page: number; limit: number; sortOrder: 'asc' | 'desc'; employeeId?: string; status?: 'PENDING' | 'APPROVED' | 'REJECTED' }) {
    const { skip, take } = toPrismaPagination(query);
    const { items, total } = await this.repo.listLeave({ skip, take, employeeId: query.employeeId, status: query.status, sortOrder: query.sortOrder });
  ...
  async decideLeave(id: string, approve: boolean, input: LeaveDecisionInput, userId: string) {
    ...
    if (current.status !== 'PENDING') throw new ConflictException(`Leave already ${current.status}`);
    assertWritten(
  ...
  async listShifts(employeeId: string, from?: string, to?: string) {
    await this.assertEmployee(employeeId);
    return this.repo.listShifts(employeeId, from ? dateOnly(from) : undefined, to ? dateOnly(to) : undefined);
  }
  ...
  async listPayslips(query: { page: number; limit: number; sortOrder: 'asc' | 'desc'; employeeId?: string; period?: string }) {
    const { skip, take } = toPrismaPagination(query);
    const { items, total } = await this.repo.listPayslips({ skip, take, employeeId: query.employeeId, period: query.period, sortOrder: query.sortOrder });
  ...
  async setPayslipStatus(id: string, input: PayslipStatusInput, userId: string) {
    ...
    assertWritten(await this.repo.updatePayslip(id, input.version, { status: input.status, generatedBy: userId }), 'Payslip');
