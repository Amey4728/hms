...
export type PurchaseWithItems = Prisma.PurchaseRequestGetPayload<{
...
export class ProcurementRepository {
  ...
  adjustItemQuantity(id: string, delta: number, userId: string): Promise<InventoryItem> {
    return this.prisma.inventoryItem.update({
      where: { id },
      data: { quantity: { increment: delta }, updatedBy: userId },
    });
  }
  lowStockItems(): Promise<InventoryItem[]> {
    return this.prisma
      .$queryRaw`SELECT * FROM inventory_items WHERE "deletedAt" IS NULL AND "isActive" = true AND quantity <= "reorderLevel" ORDER BY name`;
  }
