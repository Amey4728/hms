...
export class InventoryService {
  ...
  async listVendors(q: Q & { search?: string }) {
    ...
    const { items, total } = await this.repo.listVendors({
      skip,
      take,
      search: q.search,
  ...
  async updateVendor(id: string, input: UpdateVendorInput, userId: string) {
    ...
    assertWritten(
      await this.repo.updateVendor(id, version, { ...changes, updatedBy: userId }),
      'Vendor',
    );
  ...
  async createItem(input: CreateInventoryItemInput, userId: string) {
    return toItemView(
      await this.repo.createItem({ ...input, createdBy: userId, updatedBy: userId }),
    );
  }
  ...
  async listItems(q: Q & { search?: string; category?: InventoryCategory }) {
    const { skip, take } = toPrismaPagination(q);
    const { items, total } = await this.repo.listItems({
      skip,
      take,
      search: q.search,
      category: q.category,
  ...
  async updateItem(id: string, input: UpdateInventoryItemInput, userId: string) {
    ...
    assertWritten(
      await this.repo.updateItem(id, version, { ...changes, updatedBy: userId }),
      'Item',
    );
  ...
  async createPurchase(input: CreatePurchaseRequestInput, userId: string) {
    for (const line of input.items) {
      if (!(await this.repo.findItem(line.itemId)))
        throw new BadRequestException(`Item ${line.itemId} not found`);
    }
    if (input.vendorId && !(await this.repo.findVendor(input.vendorId)))
  ...
  async listPurchases(q: Q & { status?: PurchaseStatus }) {
    const { skip, take } = toPrismaPagination(q);
    const { items, total } = await this.repo.listPurchases({
      skip,
      take,
      status: q.status,
      sortOrder: q.sortOrder,
  ...
  approvePurchase(id: string, input: PurchaseDecisionInput, userId: string) {
    return this.purchaseTransition(id, input.version, 'APPROVED', ['SUBMITTED'], userId, {
      decisionNote: input.decisionNote,
    });
  }
  rejectPurchase(id: string, input: PurchaseDecisionInput, userId: string) {
    return this.purchaseTransition(id, input.version, 'REJECTED', ['SUBMITTED'], userId, {
      decisionNote: input.decisionNote,
    });
  }
  async receivePurchase(id: string, version: number, userId: string) {
    ...
    if (current.status !== 'APPROVED')
      throw new ConflictException('Only APPROVED requests can be received');
    assertWritten(await this.repo.receivePurchase(id, version, userId), 'Purchase request');
    return this.getPurchase(id);
  }
  private async purchaseTransition(
    id: string,
    version: number,
    to: PurchaseStatus,
    allowedFrom: PurchaseStatus[],
    userId: string,
    ...
    assertWritten(
      await this.repo.updatePurchase(id, version, { status: to, updatedBy: userId, ...extra }),
      'Purchase request',
    );
    return this.getPurchase(id);
  }
  ...
  async listTransfers(q: Q & { status?: TransferStatus }) {
    const { skip, take } = toPrismaPagination(q);
    const { items, total } = await this.repo.listTransfers({
      skip,
      take,
      status: q.status,
      sortOrder: q.sortOrder,
  ...
  async completeTransfer(id: string, version: number) {
    ...
    if (current.status !== 'PENDING')
      throw new ConflictException(`Transfer already ${current.status}`);
    assertWritten(
      await this.repo.updateTransfer(id, version, { status: 'COMPLETED', completedAt: new Date() }),
      'Transfer',
    );
