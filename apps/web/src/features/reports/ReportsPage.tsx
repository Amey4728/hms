      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <Card className="p-5">
          ...
          {revenue.data && (
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
              <span className="text-slate-500">Invoices</span><span className="text-right">{revenue.data.invoices.count}</span>
              <span className="text-slate-500">Billed</span><span className="text-right">{money(revenue.data.invoices.billed)}</span>
              <span className="text-slate-500">Collected</span><span className="text-right text-emerald-600">{money(revenue.data.invoices.collected)}</span>
              <span className="text-slate-500">Outstanding</span><span className="text-right text-amber-600">{money(revenue.data.invoices.outstanding)}</span>
              <span className="text-slate-500">Tax collected</span><span className="text-right">{money(revenue.data.invoices.tax)}</span>
              <span className="font-medium text-slate-700">Pharmacy sales</span><span className="text-right font-medium">{money(revenue.data.pharmacy.revenue)}</span>
            </div>
          )}
        </Card>
        ...
        <Card className="p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-800">Appointments by status</h3>
          </div>
          {appts.data && (
            ...
            <ul className="space-y-2 text-sm">
              {doctors.data?.map((d) => (
                <li key={d.doctorId} className="flex justify-between">
                  <span className="text-slate-700">Dr. {d.doctorName}</span>
        ...
        <Card className="p-5">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-800">Pharmacy inventory</h3>
          </div>
          {inventory.data && (
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm">
              <span className="text-slate-500">Medicines</span><span className="text-right">{inventory.data.medicines}</span>
              <span className="text-slate-500">Low stock</span><span className="text-right text-red-600">{inventory.data.lowStock}</span>
              <span className="text-slate-500">Expiring (30d)</span><span className="text-right text-amber-600">{inventory.data.expiringBatches}</span>
              <span className="font-medium text-slate-700">Stock value</span><span className="text-right font-medium">{money(inventory.data.stockValue)}</span>
            </div>
          )}
        </Card>
      </div>
